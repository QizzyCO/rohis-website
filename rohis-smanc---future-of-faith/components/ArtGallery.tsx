...
const ArtGallery: React.FC = () => {
  return (
    <section id="gallery" className="py-24 bg-slate-900/20">
      <div className="container mx-auto px-6">
        <div className="text-center mb-20">
          <div className="inline-block px-4 py-1 glass rounded-full text-blue-400 text-xs font-bold uppercase tracking-[0.3em] mb-4">
            Aesthetic Transcendence
          </div>
          <h2 className="font-serif text-4xl md:text-5xl text-slate-100 mb-6">Sacred <span className="text-blue-500">Geometry</span></h2>
          <p className="max-w-2xl mx-auto text-slate-400">
            Exploring the visual language of the divine through high-fidelity digital interpretations of 
            classical Kufic calligraphy and geometric tessellations.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {GALLERY.map((art, idx) => (
            <div 
              key={art.id} 
              className="holographic-card group relative aspect-[4/5] rounded-[2.5rem] overflow-hidden cursor-pointer"
            >
              <img 
                src={art.image} 
                alt={art.title} 
              ...
              <div className="absolute inset-0 p-10 flex flex-col justify-end translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                ...
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest text-white">Expand View</span>
                </div>
              </div>
        ...
        <div className="mt-16 text-center">
          <button className="px-8 py-4 glass text-slate-400 hover:text-white hover:bg-white/5 rounded-2xl font-bold uppercase tracking-widest transition-all">
