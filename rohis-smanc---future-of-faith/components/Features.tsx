    ...
    <section id="community" className="py-24 bg-emerald-50/30">
      <div className="container mx-auto px-6">
        <div className="text-center mb-20">
          <h2 className="font-serif text-4xl md:text-5xl text-slate-800 mb-6">Ekosistem <span className="text-emerald-600">Kami</span></h2>
          <p className="max-w-2xl mx-auto text-slate-600 font-medium">
            Kami membangun rangkaian alat yang kuat untuk memberdayakan gaya hidup Muslim modern, memadukan konektivitas dengan kontemplasi.
          </p>
        </div>
          ...
          {FEATURES.map((item) => (
            <div key={item.id} className="relative group overflow-hidden rounded-[2rem] glass p-10 h-full shadow-sm hover:shadow-2xl hover:shadow-emerald-900/10 transition-all border-emerald-50">
              ...
              <a href="#" className="inline-flex items-center text-emerald-600 text-sm font-bold uppercase tracking-widest hover:gap-2 transition-all">
                Pelajari Lebih Lanjut <span className="ml-1">→</span>
              </a>
            </div>
          ))}
        </div>
        ...
        <div className="mt-24 grid grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            { label: 'Komunitas', value: 450, suffix: '+' },
            { label: 'Jaringan Ulama', value: 1.2, suffix: 'k', decimals: 1 },
            { label: 'Anggota Global', value: 2.5, suffix: 'jt', decimals: 1 },
            { label: 'Tahun Visi', value: 25, suffix: '' }
          ].map((stat, i) => (
