...
const Assistant: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', text: response || "Maaf, saya tidak bisa memproses permintaan tersebut." }]);
    } catch (err) {
      console.error("Chat interaction failed:", err);
      setMessages(prev => [...prev, { 
        role: 'model', 
        text: "Terjadi kesalahan teknis yang tidak terduga. Mohon coba lagi sebentar lagi." 
      }]);
    } finally {
      setIsLoading(false);
    }
